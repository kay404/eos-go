...
const SignatureSM2Prefix = "SIG_SM2_"
...
func (s Signature) Validate() error {
	...
	if s.Curve == CurveK1 || s.Curve == CurveR1 || s.Curve == CurveSM2{
...
func NewSignatureFromData(data []byte) (Signature, error) {
	...
	case CurveSM2:
		signature.inner = &innerSM2Signature{}
	case CurveWA:
...
var sigReaderManifests = map[string]sigReaderManifest{
	...
	SignatureSM2Prefix: sigReaderManifest{CurveSM2, newInnerSM2Signature},
...
func NewSignature(signature string) (out Signature, err error) {
	...
	prefix := signature[0: strings.LastIndexAny(signature, "_") + 1]
	...
	verifyChecksum := Ripemd160checksumHashCurve(content, manifest.curveID.String())
