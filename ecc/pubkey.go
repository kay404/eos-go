...
var PublicKeySM2Prefix = "PUB_SM2_"
...
var pubKeyReaderManifests = map[string]pubkeyReaderManifest{
	...
	PublicKeySM2Prefix:    pubkeyReaderManifest{CurveSM2, newInnerSM2PublicKey},
...
func NewPublicKey(pubKey string) (out PublicKey, err error) {
	...
	return out, fmt.Errorf("public key should start with [%q | %q | %q | %q] (or the old %q)", PublicKeyK1Prefix, PublicKeyR1Prefix, PublicKeySM2Prefix, PublicKeyWAPrefix, PublicKeyPrefixCompat)
}
...
func (p PublicKey) Validate() error {
	...
	if p.Curve == CurveK1 || p.Curve == CurveR1 || p.Curve == CurveSM2{
...
func Ripemd160checksumHashCurve(in []byte, salt string) []byte {
	h := ripemd160.New()
	_, _ = h.Write(in) // this implementation has no error path
	...
	_, _ = h.Write([]byte(salt)) // conditionally ?
	sum := h.Sum(nil)
	return sum[:4]
}
...
func (p PublicKey) String() string {
	...
	hash := ripemd160checksum(data, p.Curve.String())
