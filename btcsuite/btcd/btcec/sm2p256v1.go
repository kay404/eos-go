...
// ParsePubKeySM2 parses a public key for a koblitz curve from a bytestring into a
// ecdsa.Publickey, verifying that it is valid. It supports compressed,
// uncompressed and hybrid signature formats.
func ParsePubKeySM2(pubKeyStr []byte, curve *Sm2P256Curve) (key *PublicKey, err error) {
	pubkey := PublicKey{}
	pubkey.Curve = curve
	...
	case PubKeyBytesLenCompressed:
		...
		pubkey.X = new(big.Int).SetBytes(pubKeyStr[1:33])
		pubkey.Y, err = decompressPointSM2(curve, pubkey.X, ybit)
		if err != nil {
			return nil, err
		}
